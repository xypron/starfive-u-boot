...
static int starfive_serial_setbrg(struct udevice *dev, int baudrate)
{
	int ret;
	struct clk clk;
	struct starfive_uart_platdata *platdata = dev_get_platdata(dev);
	u32 clock = 0;
...
static int starfive_serial_ofdata_to_platdata(struct udevice *dev)
{
	struct starfive_uart_platdata *platdata = dev_get_platdata(dev);
...
U_BOOT_DRIVER(serial_starfive) = {
	...
	.ofdata_to_platdata = starfive_serial_ofdata_to_platdata,
	.platdata_auto_alloc_size = sizeof(struct starfive_uart_platdata),
